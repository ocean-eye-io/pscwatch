// src/components/dashboard/defects/DefectStatsCards.jsx
import React, { useMemo } from 'react';
import { Activity, AlertCircle, Triangle } from 'lucide-react';

const DefectStatsCards = ({ data = [] }) => {
  const stats = useMemo(() => {
    // Count totals
    ...
    data.forEach(defect => {
      ...
      if (status !== 'CLOSED' && defect.targetDate) {
        const targetDate = new Date(defect.targetDate);
        if (targetDate < today) {
          overdueCount++;
        }
      }
        ...
        {stats.equipmentData.length > 0 && (
          <div className="mt-4 pt-3 border-t border-white/10">
            <div className="text-xs text-white/60 mb-2">
              Equipment Types: <span className="text-white/80">
                {stats.equipmentData && Object.keys(stats.equipmentData).length > 0 ? Object.keys(stats.equipmentData).length : 0}
              </span>
            </div>
...
export default DefectStatsCards;